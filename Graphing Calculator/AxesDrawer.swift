...
class AxesDrawer
{
    ...
    fileprivate func drawHashmarksInRect(_ bounds: CGRect, origin: CGPoint, pointsPerUnit: CGFloat)
    {
        if ((origin.x >= bounds.minX) && (origin.x <= bounds.maxX)) || ((origin.y >= bounds.minY) && (origin.y <= bounds.maxY))
        {
            ...
            {
                let num = NSNumber(value: Float((origin.x-bbox.minX)/pointsPerUnit))
                let label: String = formatter.string(from: num)!
                if let leftHashmarkPoint = alignedPoint(x: bbox.minX, origin.y, insideBounds:bounds) {
                    drawHashmarkAtLocation(leftHashmarkPoint, .top("-\(label)"))
                }
                if let rightHashmarkPoint = alignedPoint(x: bbox.maxX, origin.y, insideBounds:bounds) {
                    drawHashmarkAtLocation(rightHashmarkPoint, .top(label))
                }
                if let topHashmarkPoint = alignedPoint(x: origin.x, bbox.minY, insideBounds:bounds) {
                    drawHashmarkAtLocation(topHashmarkPoint, .left(label))
                }
                if let bottomHashmarkPoint = alignedPoint(x: origin.x, bbox.maxY, insideBounds:bounds) {
